...
export default {
  ...
  setup() {
    ...
    let totalPrice = computed(() => {
      const sum = state.list
        .filter((item) => state.cart_ids.includes(item.product_id))
        .reduce((total, item) => {
    ...
    const init = () => {
      if (store.state.user.isLogin) {
        ...
        getCartList().then((result) => {
          ...
          state.cart_ids = result.data
            .filter((n) => n.is_checked == 1)
            .map((item) => item.product_id);
          state.checkedAll =
            result.data.length && state.list.length == state.cart_ids.length
              ? true
              : false;
    ...
    const _motifyNumCart = (id, num) => {
      ...
      motifyNumCart(id, data1).then((result) => {
        // 修改本地存储列表的数量
    ...
    onActivated(() => {
      ...
      if (
        state.list.length != store.state.cartCount ||
        store.state.isClickAddCart
      ) {
        init();
      }
    ...
    const isCheckedOnChange = (values) => {
      ...
      changeCheckedStatus(data1);
    };

    // 全选
    const allCheckedOnClick = () => {
      if (!state.checkedAll) {
        state.cart_ids = state.list.map((item) => item.product_id);
