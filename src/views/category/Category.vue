...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      // 初始化网络请求商品数据
      const _getGoodsData = (type, category_children_id, load = false, title = '') => {
        if (load) {
          // 清空列表数据
          state.finished = true;
        }
        const page = state.goods[type].page + 1;
        getGoodsData(type, page, category_children_id, title).then(result => {
          
          for (var i in result.goods) {
            state.goods[type].list.push(result.goods[i])
          }
          // state.goods[type].list.push(...result.goods);
