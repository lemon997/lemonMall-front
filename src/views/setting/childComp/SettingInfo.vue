...
export default {
  ...
  emits: ["infoLeftOnClick", "nameOnSubmit", "avatarAfterRead"],
  ...
  setup(props, { emit }) {
    ...
    const nameOnSubmit = (values) => {
      store.dispatch("updataName", values.name).then((result) => {
    ...
    // 修改头像
    const afterRead = () => {
      emit("avatarAfterRead", state.fileList[0].file);
    };
