<template>
  <div class="list" v-if="list.length">
    <van-swipe-cell v-for="item in list" :key="item.customer_addr_id">
      <div class="list-item" @click="selectAddressOnClick(item.customer_addr_id)">
        <div class="info">
          <p>
        ...
        <div class="info-edit" @click="editOnClick(item.customer_addr_id)">
          <van-icon name="edit"/>
        </div>
      </div>
      <template #right>
        <van-button v-show="!item.is_default" square text="设为默认" class="delete-button" @click="defaultOnClick(item.customer_addr_id)" />
        <van-button square text="删除" type="danger" class="delete-button" @click="delOnClick(item.customer_addr_id)" />
      </template>
    </van-swipe-cell>
  </div>
</template>
